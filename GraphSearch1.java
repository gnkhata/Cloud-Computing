     ...
     //     + "]");
      ...
      if (node.getColor() == Node.Color.GRAY) {
        for (int v : node.getEdges()) {
	      ...
	      if(w.hasNext())
		    ww=(int)w.next();
	      //System.out.println("w.next:"+ ww);
          vnode.setDistance(node.getDistance()+ww);
          //vnode.setCurrentDistance(node.getDistance()+ww);
	      //if(vnode.getId() == 6)
	      //	System.out.println("distance:"+ node.getDistance()+" currentDistance:"+node.getCurrentDistance() );
	       List<Integer> childRoute = new ArrayList<>(node.getRoute());
	       childRoute.add(vnode.getId());
          vnode.setColor(Node.Color.GRAY);
          vnode.setRoute(childRoute);
